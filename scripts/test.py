...
max_gen = 30
max_eligible = 10
crossover_method = crossover_type.two_point
selection_method = selection_type.tournament_selection
k = -1
...
while(unsolved_flag):
    ...
    print("Guess", str(guess_count) + ":", new_output.count("B"), new_output.count("W"), new_output)
    output_list.append(new_output)

    #checking if solved
    if(new_output.count("B") == code_length):
        unsolved_flag = False
print("SOLVED:", new_guess)
if show_fitness:
